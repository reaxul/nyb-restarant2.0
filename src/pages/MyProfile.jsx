...
const MyProfile = () => {
  ...
  const handleStatusUpdate = async () => {
    try {
      await updateOrderStatus({ orderId: selectedOrder.id, status: "cancelled" }).unwrap()
              ...
              <div className="bg-gray-800 rounded-lg overflow-hidden">
                  ...
                  {items.map((item) => (
                    <div key={item.itemId} className="flex items-center space-x-4">
                      ...
                      <div className="text-right">
                        <p className="text-lg font-medium">{(item.price * item.quantity).toFixed(2)} Taka</p>
                      </div>
                    </div>
                  ))}
                </div>
                <div className="border-t border-gray-700 p-4 flex justify-between">
                  ...
                  <p className="text-lg font-bold">
                    {items.reduce((sum, item) => sum + item.price * item.quantity, 0).toFixed(2)} Taka
                  </p>
                </div>
              </div>
            </div>
        ...
        {activeTab === "orders" && (
          <div className="p-6">
            {isLoading ? (
              ...
              <p className="text-center text-xl">You haven't placed any orders yet.</p>
            ) : (
              <div className="space-y-6">
                {orders.map((order) => (
                  <div key={order.id} className="border border-gray-700 rounded-lg hover:border-gray-500">
  ...
  <div className="text-right font-medium mt-2">{order.total.toFixed(2)} Taka</div>
</div>
